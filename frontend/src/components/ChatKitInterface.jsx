...
// Mock ChatKit SDK - In a real implementation, this would be the actual ChatKit library
// Since ChatKit is not a standard library, I'm creating a mock implementation
// that follows common chat SDK patterns
class MockChatKit {
  constructor(options) {
    this.options = options;
    this.conversationId = options.conversationId || 'default-conversation';
    this.messages = [];
  }

  async connect() {
    // Simulate connecting to a chat service
    console.log('Connected to ChatKit service');
    return Promise.resolve();
  }

  async subscribeToRoom(roomId, callbacks) {
    // Simulate subscribing to a room
    console.log(`Subscribed to room: ${roomId}`);
    // In a real implementation, this would set up real-time message handling
  }

  async sendMessage({ text, roomId }) {
    // Simulate sending a message through the SDK
    const message = {
      id: Date.now(),
      text,
    ...
    // In a real implementation, this would send to the actual service
    // For now, we'll just return the message
    return Promise.resolve(message);
  }

  async getUserMessages({ roomId, limit = 20 }) {
    // Simulate getting messages from the service
    return Promise.resolve(this.messages.slice(-limit));
  }
}
...
const ChatKitInterface = ({ conversationId = 'default-conversation', isEmbedded = false }) => {
  ...
  useEffect(() => {
    const initChatkit = async () => {
      try {
        const instance = new MockChatKit({ conversationId });
        await instance.connect();
        setChatkitInstance(instance);

        // Subscribe to room for real-time messages
        await instance.subscribeToRoom(conversationId, {
          onMessage: (message) => {
  ...
  const handleSend = async (e) => {
    e.preventDefault();
    if (!inputValue.trim() || isLoading || !chatkitInstance) return;
    ...
    try {
      // In a real implementation, we would use the ChatKit SDK to send the message
      // For now, we'll simulate by calling our backend API directly
      const requestBody = {
        conversation_id: conversationId,
        message: actualQuestion,
        selected_text: isSelectionQuery ? currentSelectedText : null
      };
    ...
    <div style={{
          ...
          >
            <div style={{
              ...
              backgroundColor: message.senderId === 'current-user'
                ? 'rgba(0, 204, 68, 0.2)'
                : 'rgba(0, 255, 65, 0.1)',
              border: message.senderId === 'current-user'
                ? '1px solid rgba(0, 204, 68, 0.3)'
                : '1px solid rgba(0, 255, 65, 0.2)',
              color: message.senderId === 'current-user' ? '#00ff41' : '#00ff41',
              wordWrap: 'break-word',
              lineHeight: '1.5'
            }}>
      ...
      <div style={{
        ...
        <div style={{
          ...
          Powered by ChatKit SDK, FastAPI, Neon Postgres & Qdrant
        </div>
      </div>
